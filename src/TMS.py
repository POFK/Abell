...
#       #========================================
    ...
    def PeakFinder2(self,datag,sigma=0.001,R=1.3,N=512,Aperture=0.15,MassLimit=5.*10**13):
        # ordered by Gaussian smoothing!
        '''
        run as:
        for sigma in np.linspace(0.001,0.15,15,endpoint=False):
            tms.PeakFinder(datag,sigma=sigma)
        ...
        pdata=np.empty([len(peakind),4],dtype=np.float32)
        pdata[:,:2]=peakind[:]
        indexarr=np.zeros(shape=[N,N])
        indexarr_x=np.arange(N)[:,None]+indexarr
        indexarr_y=np.arange(N)[None,:]+indexarr
        for i in np.arange(len(peakind)):
            bool=((peakind[i,0]-indexarr_x)**2.+(peakind[i,1]-indexarr_y)**2.)<Dindex**2.
            pdata[i,2]=datag[bool].sum()
            pdata[i,3]=datas[peakind[i,0],peakind[i,1]]
        #added in 0317
        pdata=pdata[pdata[:,2]>MassLimit]
        #added in 0317
        pdata=pdata[np.argsort(pdata[:,3])[::-1]] # 3: smoothing peak; 2: TopHat peak
